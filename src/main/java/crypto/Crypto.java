...
public class Crypto {
    ...
    private static final int SCRYPT_ITERATIONS_COUNT = 131072;
    ...
    public static boolean encryptStringToFile(File f, String plaintext, char[] password) {
        ...
        boolean result = encryptFile(f, plaintextBytes, password);
        erase(plaintextBytes);
        return result;
    }
    ...
    public static boolean encryptFile(File f, byte[] plaintext, char[] password) {
        ...
        boolean result = encryptFile(f, plaintext, key, salt, macKey, macSalt);
        erase(key);
        erase(salt);
        erase(macKey);
        erase(macSalt);
     ...
     * @return true if successful, otherwise false
     */
    public static boolean erase(SecretKey key) {
        try {
            key.destroy();
            return true;
